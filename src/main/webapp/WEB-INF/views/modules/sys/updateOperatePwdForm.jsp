...
<body>
<form  name="updateOperatePwdForm" action="javascript:void(0);" id="userForm" class="form form-horizontal">
	<input type="hidden" id="id" name="id" value="${user.id }">
	<div class="form-group normal-form">
    	<label class="col-xs-12 col-sm-2 control-label  pl-0 pr-5">身份：</label>
	...
	<div class="form-group normal-form">
    	...
    	<div class="col-xs-12 col-sm-7 pl-0">
    		<input type="password" name="oldPassword" placeholder="请输入原操作密码！" id="oldPassword" datatype="*" nullmsg="请输入原密码！" 
	...
	<div class="form-group normal-form">
    	...
    	<div class="col-xs-12 col-sm-7 pl-0">
    		<input type="password" name="newPassword" id="newPassword" datatype="*6-18" placeholder="请输入新操作密码！" nullmsg="请输入新密码！"
    		autocomplete="off" class="form-control">
    	</div>
    	<div class="col-xs-12 col-sm-3 valid-msg"></div>
	</div>
	<div class="form-group normal-form">
    	...
    	<div class="col-xs-12 col-sm-7 pl-0">
			<input type="password" errormsg="您两次输入的密码不一致！" id="confirmpassword" 
			placeholder="确认密码" recheck="newPassword" ignore="ignore" datatype="*" class="form-control">
    	</div>
    	<div class="col-xs-12 col-sm-3 valid-msg"></div>
	</div>
	<div class="form-group normal-form">
...
var updateOperatePwdFormSvc = {
		url: {
			updateOperatePwd : rootPath + "/sys/home/updateOperatePwd?t="+new Date().getTime()
		},
		fnCommit: function(){
			var updateOperatePwdJson = Svc.formToJson($("#userForm"));
			
        	Svc.AjaxForm.post(updateOperatePwdFormSvc.url.updateOperatePwd,updateOperatePwdJson,function(response){
        		if(response == true){
        			layer.alert('操作密码修改成功！',function(index){
        				layer.close(index);
        				API.fnHideForm("updateOperatePwdForm");
        			});
        		}else{
        			layer.alert("原操作密码错误！！请重新输入...")
        		}
        	});
		},
		fnRegisterEvent: function(){
		    ...
		    // 表单验证
		    $("form[name='updateOperatePwdForm']").Validform({
		        ...
		        beforeSubmit: function (curform) {
		        	...
		        	updateOperatePwdFormSvc.fnCommit();
		            return false;
		        }
		    });
		}
}
updateOperatePwdFormSvc.fnRegisterEvent();
</script>
</body>